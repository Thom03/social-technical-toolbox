...
@section('content')
    <div class="container-fluid">
        <div class="form-head d-flex mb-3  mb-lg-5   align-items-start">
            <a href="{{ route('add_resource') }}" class="btn btn-danger">+ Add Resource</a>
...
{{--                                                        <span class="badge badge-primary">{{ $impactArea->name }}</span>--}}
                                                    <label class="form-check-label">
                                                        <span class="badge badge-primary">{{ $res->link }}</span>
                                                    </label>
...
{{--                                                @endif--}}
                                                ...
                                                <form action="{{ route('delete_dataset', ['id' => $res->id]) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    {{--                                                <a class="dropdown-item text-danger">Delete</a>--}}
                                                    <a href="{{ route('edit_dataset', $res->id) }}" type="button" class="btn btn-rounded btn-dark">Edit</a>
                                                    <a href="{{ route('datset_detail', $res->id) }}" type="button" class="btn btn-rounded btn-light">View</a>
